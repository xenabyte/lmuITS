...
class DocumentaryPage extends StatefulWidget {
  static const id = 'documentary_page';
...
class _DocumentaryPageState extends State<DocumentaryPage> {
  ...
  void initState() {
    super.initState();
    getDocumentaryChannels();
  }

  int _currentIndex = 4;
  int carouselIndex = 0;
  String pageName = 'documentary';
  List documentaryChannels = [];

  void getDocumentaryChannels() async {
    NetworkHelper networkHelper = NetworkHelper(kUrl + pageName);
    List data = await networkHelper.getData();
    setState(()  {
      documentaryChannels = data;
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: Container(
          child: documentaryChannels.length == 0 ? Center(
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                                ...
                                child: Container(
                                    decoration: new BoxDecoration(
                                        ...
                                        image: new DecorationImage(
                                            image: new NetworkImage(kMediaPath+documentaryChannels[itemIndex]['channel_image']),
                                            fit: BoxFit.cover
                                        )
                                    )
                                ),
                              ...
                              ListTile(
                                ...
                                title: Text(documentaryChannels[itemIndex]['channel_name'], style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),),
                              ),
                            ],
                          )
                      ),
